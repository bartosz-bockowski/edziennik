        ...
        <td class="card-body">
            <c:if test="${teacher.subjects.size() > 0}">
                <c:forEach items="${subjects}" var="subject">
                    <div>${subject.name} <a class="confirm" href="/admin/teacher/${teacher.id}/removeSubject?subjectId=${subject.id}">
            ...
            <form class="confirm" method="get" action="/admin/teacher/${teacher.id}/addSubject">
            ...
            <c:if test="${teacher.supervisedClasses.size() > 0}">
                <c:forEach items="${teacher.supervisedClasses}" var="schoolClass">
                    ${schoolClass.name} (ID: ${schoolClass.id}) <a class="confirm"
                                                                   href="/admin/teacher/${teacher.id}/removeSupervisedClass?schoolClassId=${schoolClass.id}">
                    <spring:message code="teacher.removeSupervisedClass"/>
                    <p class="msg"><spring:message code="teacher.confirmRemoveClass"/>&#13;(${schoolClass.name}, ID: ${schoolClass.id})</p>
                </a><br/>
                </c:forEach>
            </c:if>
            <form class="confirm" method="get" action="/admin/teacher/${teacher.id}/addSupervisedClass">
                <p class="msg"><spring:message code="teacher.confirmAddSupervisedClass" arguments="${teacher.fullNameWithId}"/></p>
